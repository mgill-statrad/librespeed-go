...
func runLibrespeed(runner CommandRunner, localJSONPath string) (*LibrespeedResult, error) {
	log.Println("Running librespeed-cli...")

	args := []string{"--telemetry-level", "basic", "--json"}
	if localJSONPath != "" {
		args = append(args, "--local-json", localJSONPath)
	}

	output, err := runner.Run("librespeed-cli", args...)
	log.Printf("librespeed-cli raw output: %s", string(output)) // Log output regardless of error
...
func main() {
	...
	localJSONPath := flag.String("local-json", "", "Path to JSON file with server list for librespeed-cli")
	flag.Parse()
	...
	result, err := runLibrespeed(&DefaultRunner{}, *localJSONPath)
	if err != nil {
